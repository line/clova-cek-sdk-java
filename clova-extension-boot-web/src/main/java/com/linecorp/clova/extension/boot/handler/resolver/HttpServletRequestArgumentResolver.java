...
/**
 * {@link CEKRequestHandlerArgumentResolver} for {@link HttpServletRequest}.
 */
public class HttpServletRequestArgumentResolver implements CEKRequestHandlerArgumentResolver {

    @Override
    public boolean supports(MethodParameter methodParam) {
        return HttpServletRequest.class.isAssignableFrom(methodParam.getParameterType());
    }

    @Override
    public Object resolve(MethodParameter methodParam, CEKRequestMessage requestMessage) {
        return RequestUtils.getCurrentHttpRequest();
    }
